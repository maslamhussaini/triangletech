  ...
  return <ChatContext.Provider value={{ openChat }}>
    ...
    {stage !== "closed" && !minimized && <div ref={dialogRef} className="chat-window" role="dialog" aria-modal="true" aria-label={product ? `Chat about ${product.name}` : "Chat with TriangleTech"}>
      ...
      {stage === "lead" && <form className="chat-lead-form" onSubmit={submitLead} noValidate>
        ...
        <div className="field"><label htmlFor="chat-lead-name">Full name <span>*</span></label><input ref={nameInputRef} id="chat-lead-name" value={lead.name} onChange={e => setLead({ ...lead, name: e.target.value })} placeholder="Full name" aria-invalid={!!leadErrors.name} maxLength={100} />{leadErrors.name && <span className="field-error">{leadErrors.name}</span>}</div>
        <div className="field"><label htmlFor="chat-lead-email">Email address <span>*</span></label><input id="chat-lead-email" type="email" value={lead.email} onChange={e => setLead({ ...lead, email: e.target.value })} placeholder="you@company.com" aria-invalid={!!leadErrors.email} maxLength={100} />{leadErrors.email && <span className="field-error">{leadErrors.email}</span>}</div>
        <div className="field"><label htmlFor="chat-lead-phone">Phone / WhatsApp number <span>*</span></label><input id="chat-lead-phone" type="tel" value={lead.phone} onChange={e => setLead({ ...lead, phone: e.target.value })} placeholder="+92 3XX XXXXXXX" aria-invalid={!!leadErrors.phone} maxLength={30} />{leadErrors.phone && <span className="field-error">{leadErrors.phone}</span>}</div>
        <div className="field"><label htmlFor="chat-lead-business">Business name <span className="optional">optional</span></label><input id="chat-lead-business" value={lead.businessName} onChange={e => setLead({ ...lead, businessName: e.target.value })} placeholder="Your business name" maxLength={160} /></div>
        ...
        <button className="button button-accent" type="submit">Talk to Us<Icon name="arrow" /></button>
